...
class NationalInstrumentsXSeries(Base, SlowCounterInterface, ConfocalScannerInterface, ODMRCounterInterface):
    ...
    _default_clock_frequency = ConfigOption('default_clock_frequency', 1, missing='info')
    ...
    _default_scanner_clock_frequency = ConfigOption(
        'default_scanner_clock_frequency', 1, missing='info')
    _scanner_clock_channel = ConfigOption('scanner_clock_channel', missing='warn')
    _pixel_clock_channel = ConfigOption('pixel_clock_channel')
    ...
    _odmr_trigger_line = ConfigOption('odmr_trigger_line', missing='warn')
    _odmr_switch_line = ConfigOption('odmr_switch_line', missing='warn')
    ...
    def set_voltage_range(self, myrange=None):
        ...
        if myrange is None:
            myrange = [[-9., 9.], [-9., 9.], [-10., 10.], [-10., 10.]][0:n_ch]
        ...
        try:
            ...
            for n, chan in enumerate(self._scanner_ao_channels):
                ...
                daq.DAQmxCreateAOVoltageChan(
                    ...
                    self._scanner_voltage_ranges[n][0] ,#to fix float point division
                    # maximum possible voltage
                    self._scanner_voltage_ranges[n][1],#to fix division error
                    # units is Volt
                    daq.DAQmx_Val_Volts,
    ...
    def set_up_scanner(self,
        ...
        counting, where the counts were sampled by the underlying clock.
    ...
    def _scanner_position_to_volt(self, positions=None):
        ...
        for i, v in enumerate(volts):
            if v.min() < (self._scanner_voltage_ranges[i][0] - 0.2) or v.max() > (self._scanner_voltage_ranges[i][1] + 0.2):
                self.log.error(
